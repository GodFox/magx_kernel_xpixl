...
/*****************************************************
 Date         Author         Comment
 ==========   ===========    ==========================
 21-Mar-2008  Motorola       Improve Traffic meter behavior
*******************************************************/
...
MacEventDisconnected(wlan_private * priv)
{
...
#ifdef MOTO_DBG
    PRINTM(MOTO_MSG,
           "MacEventDisconnected: Disconnected event sent to upper layer.\n");
#endif
...
#ifdef MOTO_DBG
    PRINTM(MOTO_MSG,
           "MacEventDisconnected : Disconnected, num_cmd_deauth=%d, "
           "num_event_deauth=%d, num_event_disassoc=%d, num_event_link_lost=%d.\n",
           Adapter->dbg.num_cmd_deauth, Adapter->dbg.num_event_deauth,
           Adapter->dbg.num_event_disassoc, Adapter->dbg.num_event_link_lost);
#endif
    ...
    /* stop voice and data sessions to avoid staying in full power or UAPSD when we reconnect */
...
wlan_process_rx_command(wlan_private * priv)
{
...
#ifdef MOTO_DBG
            PRINTM(INFO, "PSState set to PS_STATE_FULL_POWER - KO for DSM\n");
#endif
        } else {
...
#ifdef MOTO_DBG
            PRINTM(MOTO_MSG,
                   "CMD_RESP: BG_SCAN result ready event sent to upper layer.\n");
#else
    ...
    case HostCmd_RET_INACTIVITY_TIMEOUT_EXT:
        memmove(Adapter->CurCmd->pdata_buf, &resp->params.inactivityext,
...
send_mic_error_event(wlan_private * priv, u32 event)
{
    ...
    wireless_send_event(priv->wlan_dev.netdev, IWEVMICHAELMICFAILURE, &iwrq,
                        (u8 *) & mic);
#ifdef MOTO_DBG
    PRINTM(MOTO_MSG, "CMD_RESP: MIC error event sent to upper layer.\n");
#endif
...
#ifdef MOTO_DBG
        PRINTM(INFO, "PSState set to PS_STATE_PRE_SLEEP - KO for DSM\n");
#endif
