...
int mmc_wait_for_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if(!wait_for_completion_timeout(&complete, 200)){
		mrq->cmd->error = MMC_ERR_TIMEOUT; 
...
DBG(2,"%s: raw_csd = 0x%x, max_dtr = %d, C_SIZE = %d, CS_MULT = %d, CAP = %u, BL_LEN = %d \n", __FUNCTION__, card->raw_csd, csd->max_dtr, m, e, csd->capacity, csd->read_blkbits );
#else
		e = UNSTUFF_BITS(resp, 47, 3);
		m = UNSTUFF_BITS(resp, 62, 12);
		csd->read_blkbits = UNSTUFF_BITS(resp, 80, 4);
...
DBG(2,"%s: raw_csd = 0x%x, max_dtr = %d, C_SIZE = %d, CS_MULT = %d, CAP = %d, BL_LEN = %d \n", __FUNCTION__, card->raw_csd, csd->max_dtr, m, e, csd->capacity, csd->read_blkbits );
#endif
	} else {
		/*
...
DBG(2,"process ext csd HS BIT4 \n");
		...
		case EXT_CSD_CARD_TYPE_52 | EXT_CSD_CARD_TYPE_26:
			card->ext_csd.hs_max_dtr = 48000000;
			mmc_card_set_highspeed(card);
			break;
		case EXT_CSD_CARD_TYPE_26:
			card->ext_csd.hs_max_dtr = 24000000;
			mmc_card_set_highspeed(card); 
			break;
		default:
