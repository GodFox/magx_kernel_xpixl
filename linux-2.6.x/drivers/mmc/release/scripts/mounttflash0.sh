...
# 11/28/2007    Motorola        add .system support
...
dofsck (){
	echo "dofsck $MOUNT_NODE begin"

        if /etc/mmcfsck.sh $MOUNT_NODE
	then
	        echo "mmcfsck $MOUNT_NODE successfully"
	else
		echo "mmcfsck $MOUNT_NODE fail"
        fi
}
...
mount_part (){
	...
	then
		...
		if busybox fdisk -l ${mount_disk} | grep ${mount_node} > /dev/null
			...
			echo "$mount_disk is not availalbe.So fdisk $mount_disk"
			/etc/fdisk_mmc.sh ${mount_disk}
			echo "mkdosfs $mount_node"
