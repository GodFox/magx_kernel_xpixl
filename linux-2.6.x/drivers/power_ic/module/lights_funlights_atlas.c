 ...
 */
/*@{*/
#define ATLAS_ABMODE_INDEX    11
#define ATLAS_ABMODE_NUM_BITS 3
#define ATLAS_ABMODE_MAIN_CLI_ON  0x06
#define ATLAS_ABMODE_MAIN_ON      0x04
/*@}*/
 ...
 */
/*@{*/
#define NEVIS_KEYPAD_DC_CL  0x1E030
/*@}*/
/*******************************************************************************************
 * GLOBAL VARIABLES
 ******************************************************************************************/
...
const LIGHTS_FL_REGION_CFG_T LIGHTS_FL_region_ctl_tb[LIGHTS_FL_MAX_REGIONS] =
{
    ...
    {NULL,                                         {1, NULL}}, /* Tri Color LED #1 */
    {NULL,                                         {2, NULL}}, /* Tri Color LED #2 */
    {NULL,                                         {0, NULL}}, /* WiFi Status LED */
...
{
    /*  navi           review        number        toggle        share        trash   */
...
)    
{
    ...
    /* Update the ABMODE*/
    error = power_ic_set_reg_value(POWER_IC_REG_ATLAS_LED_CONTROL_0, ATLAS_ABMODE_INDEX ,
                                   (nStep ? ATLAS_ABMODE_MAIN_CLI_ON: ATLAS_ABMODE_MAIN_ON),
                                   ATLAS_ABMODE_NUM_BITS );
   
    error = power_ic_set_reg_mask(POWER_IC_REG_ATLAS_LED_CONTROL_2,
                                  LIGHTS_FL_CLI_DISP_DC_MASK| LIGHTS_FL_CLI_DISP_CUR_MASK,
                                  ((nStep) ? NEVIS_KEYPAD_DC_CL : 0));
...
)    
{
    ...
    error |= power_ic_set_reg_mask(POWER_IC_REG_ATLAS_LED_CONTROL_4,
                                  ...
                                  (led_onoff_tb[morping_data.mode].review_on ? led_bl_tb[LIGHTS_FL_LEDG2_REVIEW][nStep] : 0) |
...
{
    const unsigned int color_mask[3] = { LIGHTS_FL_TRI_COLOR_RED_DC_MASK,
    ...
    if((pCtlData->nData > 0)&&(pCtlData->nData <= sizeof(color_mask)/sizeof(color_mask[0])))
    {
        error = power_ic_set_reg_mask(POWER_IC_REG_ATLAS_LED_CONTROL_3,
                                      color_mask[pCtlData->nData-1],
                                      (nColor ? color_mask[pCtlData->nData-1]:0));
    }
