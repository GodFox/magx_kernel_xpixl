...
#define LICELLCON_ENABLE       1
#define LICELLCON_DISABLE      0
...
static int start_conversion(POWER_IC_ATOD_TIMING_T timing)
{
    ...
    else /* This is a hardware-timed conversion. */
    {
        ...
        setup = ((delay << ATO_SHIFT) & ATO_MASK) | ADTRIGIGN_MASK;
...
static int set_single_channel_conversion(POWER_IC_ATOD_REQUEST_T * request)
{
    ...
    {
        error |= power_ic_atod_set_therm(CHRG_THERM_ON);
        mdelay(2);
    }
    ...
    if((request->channel == POWER_IC_ATOD_CHANNEL_COIN_CELL) && (power_ic_atod_get_lithium_coin_cell_en() == 0))
    {
        ...
        error |= power_ic_atod_set_lithium_coin_cell_en(LICELLCON_ENABLE);
    }
    else if((request->channel == POWER_IC_ATOD_CHANNEL_PA_THERM) && (power_ic_atod_get_lithium_coin_cell_en() != 0))
    {
        ...
        error |= power_ic_atod_set_lithium_coin_cell_en(LICELLCON_DISABLE);
    }
...
quit_due_to_error:        
    ...
    {
        power_ic_atod_set_lithium_coin_cell_en(LICELLCON_DISABLE);
    }
